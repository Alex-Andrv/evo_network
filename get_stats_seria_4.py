...
def get_res(graph_full_name, sol):
...
for file in glob.glob('*.csv', root_dir=f"{directory_path}/{seria}"):
    ...
    with open(os.path.join(directory_path, seria, file), "r") as log:
        ...
        for line in lines:
            ...
            result = get_res(graph_full_name, solution_list)

            stats_[solver][graph_full_name].append([time, size, result])
...
for problem in all_problems:
    ...
    with open(f"{seria}/{problem}.csv", "w") as log:
            ...
            for solver in solvers:
                ...
                headers.append(f"{solver}_imp_fit")
            ...
            for i in range(max_iter):
                ...
                for solver in solvers:
                    ...
                    if len(solver_to_results[solver]) > i:
                        ...
                        line.append(solver_to_results[solver][i][2])
